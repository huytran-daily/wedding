    ...
    <div className="flex h-full cursor-pointer" onClick={handleClickInside}>
      ...
      <div className="relative flex flex-1 flex-col items-center justify-between border-r border-[#C4A08A]/40 px-5 py-10 bg-[#F5E1DA]">
        ...
        <div className="absolute top-[170px] text-center">
          <p className="text-text-wedding-primary font-times text-sm">
            {lang !== Language.en
              ? "Trân trọng báo tin"
              : "Cordially announce that"}
          </p>
          <p className="text-text-wedding-primary font-times text-sm font-semibold">
            {lang !== Language.en
              ? "Lễ Thành Hôn & Vu Quy"
              : "Wedding Ceremony & Reception"}{" "}
            <span className="font-times font-normal">
              {lang !== Language.en ? "của con chúng tôi" : "of our children"}
            </span>
          </p>
        </div>
        ...
        <div className="text-center">
          ...
          <p className="font-times text-xs">
            {lang !== Language.en
              ...
              : "(Corresponding to March 16, Year of the Fire Horse)"}
          </p>
        </div>
      </div>
      ...
      <div className="relative flex flex-1 flex-col items-center justify-between gap-0 px-5 pb-8 pt-3 bg-[#F5E1DA]">
        ...
        <div className="text-text-wedding-primary relative flex h-[80px] w-[80px] items-center justify-center gap-0.5 leading-none">
          <LaurelWreathIcon
            className="absolute -rotate-0 h-45 w-45"
            color="#7e3549"
          />
        ...
        <div className="mt-4 flex flex-col items-center justify-center gap-1">
          ...
          <p className="text-text-wedding-primary text-justify font-times text-sm px-[32px]">
            {client?.type !== "notice"
              ? lang !== Language.en
                ? `${client?.data[lang].message || ""} đến dự buổi tiệc chung vui cùng gia đình ${client?.data[lang].sayOur || "chúng tôi"} tại`
                : `${client?.data[lang].message || ""} to join us in celebrating with our family at`
              : lang !== Language.en
                ? `về ngày vui đám cưới của ${client?.data[lang].sayOur}. ${client?.data[lang].message}`
        ...
        {client?.type !== "notice" ? (
          <div className="text-center">
            ...
            <p className="font-times text-xs">
              {client?.side !== "bride"
                ? `${lang !== Language.en ? "(Nhằm ngày 16 tháng 03 năm Bính Ngọ)" : "(Corresponding to March 16, Year of the Fire Horse)"}`
                : `${lang !== Language.en ? "(Nhằm ngày 09 tháng 03 năm Bính Ngọ)" : "(Corresponding to March 09, Year of the Fire Horse)"}`}
            </p>
          </div>
        ) : (
            ...
            {successSent ? (
              lang !== Language.en ? (
                <p className="min-h-[150px] flex justify-center items-center rounded-md text-center font-times text-sm tracking-[0.08em] p-2 text-text-success">
                  Lời chúc đã được gửi! <br /> Cảm ơn{" "}
                  {client?.data[lang].groupName.toLowerCase() || "bạn"} rất
                  nhiều!
                </p>
              ) : (
                <p className="min-h-[150px] flex justify-center items-center rounded-md text-center font-times text-sm tracking-[0.08em] p-2 text-text-success">
                  Your message has been sent! <br /> Thank you so much!
                </p>
              )
            ) : (
              <textarea
        ...
        {client?.type !== "notice" ? (
          lang !== Language.en ? (
            <p className="text-center font-times text-xs uppercase tracking-[0.08em]">
              ...
              {client?.data[lang].sayOur?.toUpperCase() || "CHÚNG TÔI"} <br />
          ...
          <p className="text-center font-times text-xs uppercase tracking-[0.08em] px-6">
            MỖI LỜI CHÚC TỪ {client?.data[lang].groupName || "QUÝ VỊ"} LÀ NIỀM
            VUI VÀ SỰ TRÂN QUÝ ĐỐI VỚI {client?.data[lang].sayOur || "CHÚNG EM"}
            . XIN CHÂN THÀNH CẢM ƠN TÌNH CẢM VÀ SỰ QUAN TÂM MÀ{" "}
            {client?.data[lang].groupName || "QUÝ VỊ"} ĐÃ DÀNH.
          </p>
        ) : (
          <p className="text-center font-times text-xs uppercase tracking-[0.08em] px-6">
